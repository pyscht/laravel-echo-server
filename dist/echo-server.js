...
var EchoServer = (function () {
    ...
    EchoServer.prototype.onConnect = function () {
        ...
        this.server.io.on('connection', function (socket) {
            log_1.Log.info('connect');
            socket.onAny(function () { return log_1.Log.info('event'); });
            _this.onSubscribe(socket);
            _this.onUnsubscribe(socket);
            _this.onDisconnecting(socket);
            _this.onClientEvent(socket);
        });
        this.server.io.on('/socket.io', function (socket) {
            log_1.Log.info('connect /socket.io');
            socket.onAny(function () { return log_1.Log.info('event .io'); });
            _this.onSubscribe(socket);
            _this.onUnsubscribe(socket);
            _this.onDisconnecting(socket);
            _this.onClientEvent(socket);
        });
        this.server.io.on('/', function (socket) {
            log_1.Log.info('connect /');
            socket.onAny(function () { return log_1.Log.info('event /'); });
            _this.onSubscribe(socket);
            _this.onUnsubscribe(socket);
